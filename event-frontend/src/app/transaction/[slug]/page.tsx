...
export default function TransactionPage({
  ...
  const [pricePerSeat,setPricePerSeat] = useState(20000);
  const baseTotal = seatCount * pricePerSeat;
  const redeemValue = points;
  ...
  const checkLogin = async () => {
    ...
    const getPoint = async () => {
      try {
        const res = await fetch("http://localhost:8099/redeem/getpoint", {
          credentials: "include", // biar dapet cookie
        });
        // if (!res.ok) throw new Error("Not logged in");
    ...
    const fetchEvent = async ()=>{
      try {
        const res = await fetch(`http://localhost:8099/event/${slug}`, {
          cache: "no-store",
        });
        if (!res.ok){
          console.error("failed fetch event")
        }
        const json = await res.json();
        // backend expected shape: { success: true, data: {...} }
        setEvent(json.data)
        setPricePerSeat(json.data.price)
      } catch (err) {
